...
class EightByEight:
  ...
  def multiplexing(self, greenArray, redArray, yellowArray,count=20):
    for count in range(0,count):
        for row in range(0,8):
            #greenOrYellow = greenArray[row]|yellowArray[row]
            self.writeRowRaw(row,greenArray)
            self.writeRowRaw(row,redArray)
            self.writeRowRaw(row,yellowArray)
        time.sleep(0.0002)
